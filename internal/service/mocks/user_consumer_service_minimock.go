...
// UserConsumerServiceMock implements service.UserConsumerService
...
// NewUserConsumerServiceMock returns a mock for service.UserConsumerService
...
// RunConsumer implements service.UserConsumerService
	...
	if mmRunConsumer.RunConsumerMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmRunConsumer.t.Errorf("UserConsumerServiceMock.RunConsumer got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmRunConsumer.t.Errorf("UserConsumerServiceMock.RunConsumer got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *UserConsumerServiceMock) MinimockRunConsumerInspect() {
	for _, e := range m.RunConsumerMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to UserConsumerServiceMock.RunConsumer with params: %#v", *e.params)
		}
	}
	...
	if m.RunConsumerMock.defaultExpectation != nil && afterRunConsumerCounter < 1 {
		if m.RunConsumerMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to UserConsumerServiceMock.RunConsumer")
		} else {
			m.t.Errorf("Expected call to UserConsumerServiceMock.RunConsumer with params: %#v", *m.RunConsumerMock.defaultExpectation.params)
		}
	}
	...
	if !m.RunConsumerMock.invocationsDone() && afterRunConsumerCounter > 0 {
		m.t.Errorf("Expected %d calls to UserConsumerServiceMock.RunConsumer but found %d calls",
			mm_atomic.LoadUint64(&m.RunConsumerMock.expectedInvocations), afterRunConsumerCounter)
	}
}
